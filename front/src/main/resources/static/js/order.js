			...
			$.each(result.data, function(index, item){
				...
				html += '<div class="order-box '+(item.status<1 ? 'order-pay': '')+'">\
							<div class="order-header">\
								...
								+ ['等待付款', '等待发货', '运送中', '已收货', '申请退款','退货中', '已退货'][item.status] +
								'</div>\
							</div>\
							\
							<table class="detail">\
								<thead>\
									<tr>\
										<th class="info">'+item.createTime+' | '+(item.addr!=null?item.addr.contact:'')+' | 订单号：'+item.id+'| 在线支付（支付宝快捷支付）</th>\
						...
						$.each(item.orderDetails, function(i, detail){
							html += '<tr>\
										<td class="goods">\
											<div class="goods-box">\
												<div class="img">\
													<a href=""><img src="/upload/'+(detail.goods.pics.length>0? detail.goods.pics[0].url:'default.png')+'" ></a>\
												</div>\
										...
										<td class="actions">'+
											( item.status == 0 ? '<a class="active" href="/user/order/pay?payId='+item.id+'">立即付款</a>' : '') +
											'<a href="">订单详情</a>\
											<a href="">联系客服</a>\
										</td>\
									</tr>'
